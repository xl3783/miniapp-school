...
const initExtConfig = (app) => {
  ...
  app.globalData.baseUrl = extConfig.apiHost || "http://localhost:3000";
...
const initNetwork = async (app) => {
  const configs = networkList.map((item) => {
    if (item.key === "OE") {
      return {
        ...item,
        url: `${app.globalData.baseUrl}/miniapp`
      };
    }
    return item;
  });
...
const initData = async (app) => {
  try {
    ...
    const [userInfo, tabbar, config] = await Promise.all([getUserInfo(networkContext), getBottom(networkContext), getHomeConfig(networkContext)]);
...
const getUserInfo = (networkContext) => {
  ...
  return wx.getEnterOptionsSync().scene === 1154 ? Promise.resolve() : networkContext.get("/userInfo");
};

const getBottom = (networkContext) => {
  return networkContext.get("/bottom");
};

const getHomeConfig = (networkContext) => {
  return networkContext.get("/home/config");
};
